#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""032-indextts2 本地入口。"""

from __future__ import annotations

import argparse
import shutil
import subprocess
from pathlib import Path

EXP_DIR = Path(__file__).resolve().parent
MODAL_APP = EXP_DIR / "modal_app.py"
VOL_OUT = "modal-lab-indextts2-outputs"
VOL_PROMPT = "modal-lab-indextts2-prompts"
DEFAULT_GPU = "L4"
REF_LOCAL = EXP_DIR / "inputs" / "voices" / "ref.wav"


def _modal() -> str:
    m = shutil.which("modal")
    if not m:
        p = Path.home() / ".local" / "bin" / "modal"
        if p.is_file():
            return str(p)
        raise SystemExit("未找到 modal CLI")
    return m


def _run(cmd: list[str]) -> int:
    print("+", " ".join(cmd), flush=True)
    return subprocess.call(cmd)


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="032 IndexTTS-2 on Modal (L4)")
    sub = p.add_subparsers(dest="cmd", required=True)
    sub.add_parser("status")
    d = sub.add_parser("download")
    d.add_argument("--force", action="store_true")
    sub.add_parser("seed-prompt")
    sm = sub.add_parser("smoke")
    sm.add_argument("--gpu", default=DEFAULT_GPU)
    sm.add_argument("--kind", default="zh", choices=["zh", "en", "emo"])
    sm.add_argument("--run-name", default="")
    t = sub.add_parser("t2s")
    t.add_argument("--gpu", default=DEFAULT_GPU)
    t.add_argument("--text", required=True)
    t.add_argument("--emo-text", default="")
    t.add_argument("--run-name", default="")
    ls = sub.add_parser("ls")
    ls.add_argument("--path", default="runs")
    pl = sub.add_parser("pull")
    pl.add_argument("--remote", default="runs/smoke_zh")
    pl.add_argument("--dest", default=str(EXP_DIR / "outputs"))
    ns = p.parse_args(argv)
    m = _modal()
    if ns.cmd == "status":
        return _run([m, "run", str(MODAL_APP), "--action", "status"])
    if ns.cmd == "download":
        cmd = [m, "run", str(MODAL_APP), "--action", "download"]
        if ns.force:
            cmd.append("--force-download")
        return _run(cmd)
    if ns.cmd == "seed-prompt":
        if not REF_LOCAL.is_file():
            raise SystemExit(f"missing {REF_LOCAL}")
        return _run([m, "volume", "put", "--force", VOL_PROMPT, str(REF_LOCAL), "ref.wav"])
    if ns.cmd == "smoke":
        # ensure prompt exists on volume
        if REF_LOCAL.is_file():
            _run([m, "volume", "put", "--force", VOL_PROMPT, str(REF_LOCAL), "ref.wav"])
        cmd = [
            m,
            "run",
            "--timestamps",
            str(MODAL_APP),
            "--action",
            "smoke",
            "--gpu",
            ns.gpu,
            "--smoke-kind",
            ns.kind,
        ]
        if ns.run_name:
            cmd += ["--run-name", ns.run_name]
        return _run(cmd)
    if ns.cmd == "t2s":
        if REF_LOCAL.is_file():
            _run([m, "volume", "put", "--force", VOL_PROMPT, str(REF_LOCAL), "ref.wav"])
        cmd = [
            m,
            "run",
            "--timestamps",
            str(MODAL_APP),
            "--action",
            "t2s",
            "--gpu",
            ns.gpu,
            "--text",
            ns.text,
        ]
        if ns.emo_text:
            cmd += ["--emo-text", ns.emo_text]
        if ns.run_name:
            cmd += ["--run-name", ns.run_name]
        return _run(cmd)
    if ns.cmd == "ls":
        return _run([m, "volume", "ls", VOL_OUT, ns.path])
    if ns.cmd == "pull":
        Path(ns.dest).mkdir(parents=True, exist_ok=True)
        return _run([m, "volume", "get", "--force", VOL_OUT, ns.remote, str(ns.dest)])
    raise SystemExit(f"unknown {ns.cmd}")


if __name__ == "__main__":
    raise SystemExit(main())
