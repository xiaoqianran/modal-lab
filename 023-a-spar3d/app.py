# -*- coding: utf-8 -*-
"""023-a-spar3d — Stability SPAR3D (stable-point-aware-3d) image→GLB on Modal.

Fast feedforward mesh with point-cloud conditioning + UV materials.
Default GPU: L40S · optional RTX-PRO-6000.
License: Stability AI Community.

Default weights: zimengxiong/Modelr-SPAR3D (ungated mirror).
Official gated id: stabilityai/stable-point-aware-3d.
"""
...
APP_NAME = "modal-lab-spar3d"
DEFAULT_GPU = "L40S"
CODE_DIR = Path("/opt/src/stable-point-aware-3d")
...
VOLUME_WEIGHTS = "modal-lab-spar3d-weights"
VOLUME_OUTPUTS = "modal-lab-spar3d-outputs"

SAMPLE_URL = (
    "https://raw.githubusercontent.com/VAST-AI-Research/TripoSR/main/examples/chair.png"
)
HF_MODEL = "zimengxiong/Modelr-SPAR3D"
HF_MODEL_OFFICIAL = "stabilityai/stable-point-aware-3d"
UPSTREAM = "https://github.com/Stability-AI/stable-point-aware-3d"
...
def _build_ext_cmds() -> list[str]:
    return [
        ...
        f"git clone --depth 1 {UPSTREAM}.git stable-point-aware-3d",
        ...
        "python -c \"import torch,texture_baker,uv_unwrapper,alpha_clip; "
...
def _ensure_dirs() -> None:
    for p in (WEIGHTS / "hf" / "hub", WEIGHTS / "clip", MESHES, BENCH, INPUTS):
        p.mkdir(parents=True, exist_ok=True)
    os.environ["HF_HOME"] = str(WEIGHTS / "hf")
    os.environ["HUGGINGFACE_HUB_CACHE"] = str(WEIGHTS / "hf" / "hub")
    os.environ["HF_HUB_ENABLE_HF_TRANSFER"] = "1"
    os.environ["ALPHA_CLIP_PATH"] = str(WEIGHTS / "clip")
...
def _run_i2v(
    *,
    ...
    print("[model] load", hf_model, "low_vram=", low_vram_mode, flush=True)
    ...
    image = remove_background(Image.open(img_path).convert("RGBA"), bg_remover)
    ...
    points_path = work / f"{_safe(output_name)}_points.ply"
    try:
        pcs = glob_dict.get("point_clouds") if isinstance(glob_dict, dict) else None
        if pcs:
            pcs[0].export(str(points_path))
    except Exception as e:
        print(f"[warn] points export: {e}", flush=True)
    t_mesh = time.time() - t3
    total = time.time() - t0
    _peak()
    ...
    meta = {
        ...
        "upstream": "Stability-AI/stable-point-aware-3d",
    ...
    if points_path.is_file():
        try:
            pdest = MESHES / f"{_safe(output_name)}_points.ply"
            shutil.copy2(points_path, pdest)
            outputs_vol.commit()
        except Exception:
            pass
    try:
...
class SPAR3DWorker:
    ...
    def probe(self) -> dict:
        ...
        info = {
            ...
            "smi": _smi(),
            "code_ok": (CODE_DIR / "spar3d").is_dir(),
            "hf_token": bool(
