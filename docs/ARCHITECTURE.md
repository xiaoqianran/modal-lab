...
040/041/042 不定义 Modal App，而是 provider/integration 的客户端验证脚本，因此保留 `run.py`。三者都通过 PEP 723 声明脚本依赖；根 launcher 优先复用已有 venv，无 venv 时用 `uv run --script` 隔离执行。这是不同的入口类型，不是 legacy 兼容。

```text
普通实验                Provider / integration 验证
experiment/app.py       040|041|042/run.py
       │                       │
